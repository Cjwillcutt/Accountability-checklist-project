...
export default function NewProject() {
  ...
  function addDeadline() {
    if (newDeadlineLabel.trim() && newDeadlineDate) {
      setDeadlines((prev) => [...prev, { label: newDeadlineLabel.trim(), date: newDeadlineDate }]);
      setNewDeadlineLabel("");
      setNewDeadlineDate("");
    }
  }
  ...
  function addChecklistItem() {
    ...
    if (trimmed) {
      setChecklistItems((prev) => [...prev, { text: trimmed, done: false }]);
      setNewChecklistItem("");
    }
  }
  ...
  const actions: { id: SectionId; icon: React.ReactNode; label: string; color: string; badge?: number }[] = [
    {
      ...
      badge: invitedEmails.length || undefined,
    },
    {
      id: "deadline",
      ...
      label: "Set Deadlines",
      color: "text-primary",
      badge: deadlines.length || undefined,
    },
    {
      id: "upload",
    ...
    },
    {
      ...
      label: "Workload Checklist",
      color: "text-amber-400",
      badge: checklistItems.length || undefined,
    },
  ];
                      ...
                      >
                        <div className="bg-background/40 border border-t-0 border-border rounded-b-xl px-6 py-5">
                          ...
                          {action.id === "invite" && (
                            <div>
                              <p className="text-muted-foreground text-sm mb-3">
                                Enter your group members' email addresses to invite them.
                              </p>
                              <div className="flex gap-2 mb-3">
                                ...
                                <Button
                                  ...
                                  className="bg-secondary hover:bg-secondary/90 text-white px-4 rounded-lg shrink-0"
                                >
                              ...
                              {invitedEmails.length > 0 && (
                                <ul className="flex flex-col gap-2">
                                  {invitedEmails.map((email) => (
                                    <li key={email} className="flex items-center gap-2 text-sm text-white">
                          ...
                          {action.id === "deadline" && (
                            <div>
                              <p className="text-muted-foreground text-sm mb-3">
                                Add milestones with due dates to keep the group on track.
                              </p>
                              <div className="flex gap-2 mb-3 flex-wrap">
                                <Input
                                  ...
                                  className="h-10 bg-background border-border text-white placeholder:text-muted-foreground flex-1 min-w-32"
                                />
                                <Input
                                  type="date"
                                  ...
                                  className="h-10 bg-background border-border text-white w-40"
                                />
                                <Button
                                  onClick={addDeadline}
                                  data-testid="button-add-deadline"
                                  className="bg-primary hover:bg-primary/90 text-primary-foreground px-4 rounded-lg"
                                >
                                  <Plus className="w-4 h-4" />
                                </Button>
                              </div>
                          ...
                          {action.id === "upload" && (
                            <div>
                              <p className="text-muted-foreground text-sm mb-3">
                                Upload rubrics, outlines, or any relevant files for the group.
                              </p>
                              <label
                                className="flex flex-col items-center justify-center w-full h-24 border-2 border-dashed border-border rounded-xl cursor-pointer hover:border-purple-400/60 transition-colors bg-background/50"
                          ...
                          {action.id === "checklist" && (
                            <div>
                              <p className="text-muted-foreground text-sm mb-3">
                                Break down the project into tasks and assign them to group members.
                              </p>
                              <div className="flex gap-2 mb-3">
                                <Input
                                  ...
                                  className="h-10 bg-background border-border text-white placeholder:text-muted-foreground flex-1"
                                />
                                <Button
                                  onClick={addChecklistItem}
                                  data-testid="button-add-checklist-item"
                                  className="bg-amber-500 hover:bg-amber-500/90 text-white px-4 rounded-lg"
                                >
                                  <Plus className="w-4 h-4" />
                                </Button>
                              </div>
                                  ...
                                  {checklistItems.map((item, i) => (
                                    ...
                                    >
                                      <button
                                        ...
                                        data-testid={`button-toggle-checklist-${i}`}
                                      >
                                        {item.done && <Check className="w-3 h-3 text-primary-foreground" />}
                                      </button>
                                      <span className={`flex-1 ${item.done ? "line-through text-muted-foreground" : "text-white"}`}>
                                        {item.text}
                                      </span>
                                      <button
                                        type="button"
